...
def main(args):
    ...
    if not is_eval:
        wandb.init(project="mobile-aloha2")
...
def get_image(ts, camera_names, rand_crop_resize=False):
    ...
    for cam_name in camera_names:
        print('推理的图像格式为（学习的时候图像格式为480*640*3）：', ts[cam_name].shape)
        ...
        with torch.inference_mode():
            ...
            for t in range(max_timesteps):
                ...
                if t % query_frequency == 0:
                    curr_image = get_image(ts, camera_names, False)
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    if config['load_pretrain']:
        loading_status = policy.deserialize(torch.load(
            os.path.join('/home/Data/check_points', 'policy_step_1000_seed_0.ckpt')))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--validate_every', action='store', type=int, default=1000, help='validate_every',
                        required=False)
    ...
    parser.add_argument('--num_steps', action='store', type=int, help='num_steps', required=False,default=1000000)
